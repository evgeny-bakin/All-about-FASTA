'''
This module contains "fast_report" function, providing some basic statistics for our FASTA/FASTQC file and "quality_score" function, performing assesment of average per base quality
'''
import Bio
import statistics
import csv
...
def fast_report(input_file, output_file, output_mode="quiet"):
    with open(input_file, "r"):
        ...
        if file_type == "fasta" and count_reads == 1:
            ...
            if output_mode == "verbose":
                print(
                    "The file contains {} reads. The read lengths range from {} to {}. The length of the most reads is {}.".format(
                        count_reads, min(read_length), max(read_length), statistics.mode(read_length)))
