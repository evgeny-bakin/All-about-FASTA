...
def fast_report(input_file_name, output_file_name, output_mode = "quiet" ):
    with open(input_file_name, "r"):
        read_length = [len(seq_record.seq) for seq_record in SeqIO.parse(input_file_name, file_type)]
        count_reads = len(read_length)
        if file_type == "fasta" and count_reads == 1:
            print("Length:", read_length, file = open(output_file_name, "w"))
